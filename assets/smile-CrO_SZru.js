import{$ as a,t as M,h as N,v as B,x as S,b as $,q,e as T,c as w,a as g,l as m,g as U,d as I,M as D,n as x,j as O,i as A,N as h,y as _,z}from"./widev-C8Ez2uC0.js";const G=()=>`
...
`,Y=()=>{const e=new Date,o=[];for(let i=-2;i<=3;i++){const t=new Date(e.getFullYear(),e.getMonth()+i,1),n=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,"0")}`,v=i===0?"selected":"",d=t.toLocaleDateString("es-PE",{month:"long",year:"numeric"});o.push(`<option value="${n}" ${v}>${d.charAt(0).toUpperCase()+d.slice(1)}</option>`)}return o.join("")},H=e=>e?e.split("-").reverse().join("/"):"—",P=()=>{const e=a("#fechaIngreso").val(),o=parseInt(a("#diasReservados").val())||1;if(!e)return;const i=new Date(`${e}T00:00:00`);i.setDate(i.getDate()+o),a("#fechaSalida").val(i.toISOString().split("T")[0])},j=()=>{const e=new Date().toISOString().split("T")[0];a("#fechaIngreso").val(e),P()};let p=null,c=null,f=[],l=[],y=new Date().toISOString().slice(0,7),r=1,b=10;a(async()=>{a(".app").html(G()),M(N,async e=>{if(!e)return location.href="/";p=e,await L(),await Q(),await V(),K(),C()})});const K=()=>{j(),a("#diasReservados,#fechaIngreso").on("change",P),a("#regForm").on("submit",W),a("#mesFiltro").on("change",function(){y=a(this).val(),r=1,u(),C()}),a("#filCola,#filCant").on("change",function(){r=1,u()}),a(document).on("click",".bt_cargar",oa),a(document).on("click",".bt_salir",async()=>{await B(N);try{localStorage.clear()}catch{}location.href="/"}),a("#numDocumento").on("input",Z),a("#celular").on("input",function(){a(this).val(a(this).val().replace(/\D/g,"").slice(0,9))}),a("#precio").on("input",aa),a("#nombreCliente").on("input",function(){a(this).val(a(this).val().toUpperCase())}),a("#tipoDocumento").on("change",function(){const e={DNI:"12345678",RUC:"20123456789","Carnet Extranjeria":"001234567",Pasaporte:"ABC123456"};a("#numDocumento").val("").attr("placeholder",`Ej: ${e[a(this).val()]}`)}),a("#pagBox").on("click",".page-btn",function(){r=+a(this).data("page"),u()})},L=async()=>{const e=S("wiSmile");if(e)return c=e,R();const o=await $(q(w(g,"smiles"),T("usuario","==",p.displayName)));if(!o.empty)c=o.docs[0].data(),m("wiSmile",c,450);else{const i=await U(I(g,"smiles",p.displayName));i.exists()?(c=i.data(),m("wiSmile",c,450)):c={usuario:p.displayName,nombre:p.displayName,imagen:""}}R()},R=()=>{const e=c?.nombre||c?.usuario||p.displayName;a("#usrNom").text(e),a("#usrImg").attr("src",(c?.imagen||"").trim()?c.imagen:"./smile.png"),D(`Bienvenido ${e}`)},Q=async()=>{const e=S("empleadosSmile");if(e){f=e,k(),E();return}f=(await $(q(w(g,"smiles"),T("participa","==","si")))).docs.map(i=>({id:i.id,...i.data()})),m("empleadosSmile",f,300),k(),E()},k=()=>{if(!f.length)return a("#colGrid").html('<div class="no-colabs">Sin colaboradores</div>');const e=l.length||1,o=f.map((i,t)=>{const n=l.filter(d=>d.colaborador===i.nombre||d.colaborador===i.usuario).length,v=Math.round(n/e*100);return`
    ...
    </div>`}).join("");a("#colGrid").html(o)},E=()=>{const e=f.map(o=>`<option value="${o.nombre||o.usuario}">${o.nombre||o.usuario}</option>`).join("");a("#filCola").html(`<option value="">Todos</option>${e}`)},V=async()=>{const e=S("sm_regh");if(e){l=e,u();return}try{l=(await $(w(g,"registrosHotel"))).docs.map(i=>({id:i.id,...i.data()})),l.sort((i,t)=>new Date(t.fechaIngreso||"1970-01-01")-new Date(i.fechaIngreso||"1970-01-01")),m("sm_regh",l,300)}catch(o){console.error(o)}u()},W=async e=>{if(e.preventDefault(),!ea())return!1;try{x(!0);const o=Date.now().toString(),i={id:o,registroEn:a("#registroEn").val(),nombreCliente:a("#nombreCliente").val(),tipoDocumento:a("#tipoDocumento").val(),numDocumento:a("#numDocumento").val(),celular:a("#celular").val(),numHabitacion:a("#numHabitacion").val(),tipoHabitacion:a("#tipoHabitacion").val(),precio:parseFloat(a("#precio").val())||0,moneda:a("#moneda").val(),metodoPago:a("#metodoPago").val(),diasReservados:parseInt(a("#diasReservados").val())||1,fechaIngreso:a("#fechaIngreso").val(),fechaSalida:a("#fechaSalida").val(),tipoReserva:a("#tipoReserva").val(),bookingId:a("#bookingId").val()||"",observaciones:a("#observaciones").val()||"",colaborador:c?.nombre||c?.usuario||p.displayName,fechaRegistro:O()};await A(I(g,"registrosHotel",o),i),l.unshift(i),m("sm_regh",l,300),D("Registro guardado"),a("#regForm")[0].reset(),j(),u(),C()}catch(o){console.error(o),h("Error al guardar","error")}finally{x(!1)}return!1},J=async e=>{if(confirm("Eliminar registro?"))try{await z(I(g,"registrosHotel",e.toString())),l=l.filter(o=>o.id!=e),m("sm_regh",l,300),u(),C(),D("Registro eliminado")}catch(o){console.error(o),h("Error al eliminar","error")}},u=()=>{const e=a("#filCola").val(),o=a("#filCant").val(),i=l.filter(d=>{const s=(d.fechaIngreso||"").slice(0,7);return(!y||s===y)&&(!e||d.colaborador===e)});b=o==="all"?i.length:parseInt(o||"10");const t=Math.max(1,Math.ceil((i.length||0)/(b||1)));r>t&&(r=1);const n=(r-1)*b,v=i.slice(n,n+b);if(!v.length)a("#tabBody").html('<tr><td colspan="11" style="text-align:center;color:var(--bg2);padding:2vh">Sin datos</td></tr>');else{const d=v.map(s=>{const F=s.moneda==="Soles"?"S/":s.moneda==="Dolares"?"US$":"€";return`
      <tr>
        <td><strong>${H(s.fechaIngreso)}</strong></td>
        <td><strong>${H(s.fechaSalida)}</strong></td>
